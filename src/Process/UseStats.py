...
class UseStats:
    def getPattern(self, path, file):
        ...
        with open(path+"/"+file+".txt", encoding="utf8", errors='ignore') as f:
            ...
            for i in range(0, len(chunk_array), 2):
                ...
                parameters = re.findall(r"\{0,\d{,3},\d{,3},\d{,3}\}\{1,\d{,3},\d{,3},\d{,3}\};|\{1,\d{,3},\d{,3},\d{,3}\}\{0,\d{,3},\d{,3},\d{,3}\};", chunk_array[i+1])
                if len(parameters)!=0:
    ...
    def main(self):
        ...
        with pd.ExcelWriter(file_path+'/public/Ressource/scripts/'+"useStats_.xlsx") as writer:
